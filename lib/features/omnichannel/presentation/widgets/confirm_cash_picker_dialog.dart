...
/// BRICK 3-APP — picker ringan Konfirmasi Cash. Fetch bookings belum-lunas via
/// [onFetchBookings], pilih SATU kode, lalu `Navigator.pop<String>(bookingCode)`
/// saat Konfirmasi / `pop(null)` saat Batal. Konfirmasi + error ditangani pemanggil
/// (dashboard). Sengaja TIDAK reuse ManualPaymentComposeDialog (terkopel total/loket +
/// opsi tanpa-kode) — confirm-cash kirim {booking_code} SAJA.
class ConfirmCashPickerDialog extends StatefulWidget {
  const ConfirmCashPickerDialog({super.key, required this.onFetchBookings});

  final Future<List<Map<String, dynamic>>> Function() onFetchBookings;

  @override
  State<ConfirmCashPickerDialog> createState() =>
      _ConfirmCashPickerDialogState();
}

class _ConfirmCashPickerDialogState extends State<ConfirmCashPickerDialog> {
  bool _loading = true;
  String? _fetchError;
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Konfirmasi Cash'),
